...
//	average(x, y, c) = Halide::cast<uint8_t>(1.0f*val1+1.0f*val2)/2;
	//average(x, y, c) = input2(x,y,c); //Halide::cast<uint8_t>((Halide::cast<uint32_t>(input1(x,y,c))*1.0f + Halide::cast<uint32_t>(input2(x,y,c))*1.0f)*0.5f);
...
//	save(output, "result.png");
	...
	int w = min(w1, w2);
	int h = min(h1, h2);*/
//	Halide::Image<uint8_t> output = average.realize(min(input1.width(), input2.width()), min(input1.height(), input2.height()), min(input1.channels(), input2.channels()));

//	printf("min(input1.width(), input2.width(): %d\n",input1.width());
//	Halide::Image<uint8_t> output = average.realize(min(in);
