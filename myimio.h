// ??? /antonfs/scratch/awetzel/newjosh/LGNs1/rawMontages/W28/real_bad_top.pgm
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
fprintf(stderr, "raw fd %d\n", fd);
		if(RHDR)
			read(fd, ip->pp, RHDR);
		read(fd, ip->pp, ip->wid*ip->ht*ip->bpp);
		close(fd);
		return(ip);
	}
...
//fprintf(stderr, "fourth ftell %d\n", ftell(fp));
		ip = newimage(wid, ht, bpp);
		if(!ip)
			return(NULL);
//fprintf(stderr, "fifth ftell %d\n", ftell(fp));
		ignore = fread(ip->pp, 1, (long)ht*wid*bpp, fp);
/*
fprintf(stderr, "ignore %d  pp 0x%lx  %ld\n", ignore, ip->pp, (long)ht*wid*bpp);
{
...
write(fd, ip->pp, (long)ht*wid*bpp);
close(fd);
}
*/
	}
...
//fd, fname, ip->wid, ip->ht, ip->bpp);
	if(strstr(fname, "tif") || strstr(fname, "TIF")) {
		*(int *)(&tiff_hdr[2]) = ip->wid*ip->ht*ip->bpp+8;
		write(fd, tiff_hdr, 8);
		write(fd, ip->pp, ip->wid*ip->ht*ip->bpp);
		tiff_mfd[5] = ip->wid;
		tiff_mfd[11] = ip->ht;
		if(ip->bpp == 3)
		...
		*(int *)(&tiff_mfd[41]) = ip->wid*ip->ht*ip->bpp; // byte count
		write(fd, tiff_mfd, sizeof(tiff_mfd));
	} else {
		if(ip->bpp == 3)
			sprintf(hdr, "P6\n%d %d\n255\n", ip->wid, ip->ht);
		else
