...
int main(int argc, char **argv) {
	...
	Halide::Func flip;//The funkiest func.
    ...
    Halide::Expr temp = input(x, y, c);
    Halide::Expr other = input(max(width-x,x-width),y,c);
    ...
    // on the input image.
    Halide::Image<uint8_t> output = flip.realize(input.width()/2, input.height()/2, input.channels());

    // Save the output for inspection. It should look like a bright parrot.
    save(output, "flip.png");
